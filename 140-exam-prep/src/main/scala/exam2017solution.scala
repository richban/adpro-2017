...
// Andrzej Wąsowski
//
// Thanks to ____ for helping with
...
object Q1 { // Association lists
  ...
  // reduce for lists; It would be faster to sort the list first.
...
object Q7 {
  ...
  def concatenate[A, B >: A] (left: FingerTree[A]) (right: FingerTree[B]) :FingerTree[B] =
      reduceL[B,FingerTree[B]] { case (t,b) => t addR b } (left, right) // this could also miss some types in the question
